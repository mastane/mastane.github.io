...
I_m & 0 \\
0 & -I_S
\end{pmatrix} \nabla F( (\nabla f)^{-1}( \zeta(t) ) ) = A(\zeta(t))$.
Contrary to classic gradient flow, the function $F(\zeta(t))$ is not necessarily decreasing along the flow.
...
Hence,
$$
\frac{d}{dt}F(\zeta(t)) = \ell(x(t))^\intercal [ \nabla^2 h( (\nabla h)^{-1}(\xi(t)) ) ]^{-1} \ell(x(t))
- || J_\ell(x(t))^\intercal (\nabla h)^{-1}(\xi(t)) ||^2 ,
$$
which is a difference of two nonnegative terms.
