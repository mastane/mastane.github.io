...
( \nabla f + \lambda A )(x',q') = \nabla f(x,q)
$$
with step-size $\lambda>0$, $f(x,q)=\frac12 \|x\|^2 + h(q)$, $h(q)=\sum_{s=1}^S q_s \log(q_s)$ the negentropy, and
$$
A(x,q) = \begin{pmatrix}
J_\ell(x)^\intercal q \\
-\ell(x)
\end{pmatrix}
$$
where $J_\ell$ denotes the Jocobian matrix of $\ell:\mathbb{R}^m \rightarrow \mathbb{R}^S$ with each $\ell_s$ convex ($\forall s\in \{1,\dots,S\}$).
